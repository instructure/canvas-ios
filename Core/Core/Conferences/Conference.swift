...
final class ConferenceRecording: NSManagedObject, WriteableModel {
    ...
    static func save(_ item: APIConferenceRecording, in context: NSManagedObjectContext) -> ConferenceRecording {
        ...
        model.playbackURL = item.playback_url?.rawValue ?? item.playback_formats.first { $0.type.contains("video") }?.url.rawValue ?? item.playback_formats.first { $0.type.contains("presentation") }?.url.rawValue
