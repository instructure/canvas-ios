...
class ModuleStore: NSObject {
    ...
    func refresh(force: Bool = false) {
        ...
        let request = GetModulesRequest(courseID: courseID, include: [])
            ...
            self.database.performBackgroundTask { context in
                ...
                do {
                    ...
                    for apiModule in response where apiModule.items == nil {
                        let request = GetModuleItemsRequest(courseID: self.courseID, moduleID: apiModule.id.value, include: [.content_details, .mastery_paths])
