            ...
            Section(
                ...
                header: {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        AUI.Divider(config.headerDividerStyle)
                    }
                }
            )
...
extension AUI.CollapsibleListSection {

    public struct Config: Equatable {
        let showItemCount: Bool
        let readListItemCount: Bool
        let headerPaddingSet: AUI.Styles.PaddingSet
        let collapseIconSize: CGFloat
        let headerDividerStyle: AUI.Divider.Style
        let sectionBackgroundColor: Color

        public init(
            showItemCount: Bool = false,
            readListItemCount: Bool = true,
            headerPaddingSet: AUI.Styles.PaddingSet = .sectionHeader,
            collapseIconSize: CGFloat = 18,
            headerDividerStyle: AUI.Divider.Style = .full,
...
#Preview {
    ...
    BaseScreen(state: .data) { _ in
        VStack(spacing: 0) {
            AUI.Divider()
            AUI.CollapsibleListSection(title: "First Section", itemCount: count1, isExpanded: $isExpanded1) {
                VStack(spacing: 0) {
                    ForEach(0..<count1, id: \.self) { index in
                        ...
                        AUI.Divider(isLast: index == count1 - 1)
                    }
                }
            }
            AUI.CollapsibleListSection(title: "Second Section", itemCount: count2, isExpanded: $isExpanded2) {
                VStack(spacing: 0) {
                    ForEach(0..<count2, id: \.self) { index in
                        Text(verbatim: "Item \(index)")
                            ...
                            .frame(maxWidth: .infinity, alignment: .leading)
                            .paddingStyle(set: .standardCell)
                        AUI.Divider(isLast: index == count2 - 1)
                    }
                }
            }
        }
