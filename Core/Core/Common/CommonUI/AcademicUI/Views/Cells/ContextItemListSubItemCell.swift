//
// This file is part of Canvas.
// Copyright (C) 2025-present  Instructure, Inc.
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as
// published by the Free Software Foundation, either version 3 of the
// License, or (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program.  If not, see <https://www.gnu.org/licenses/>.
//

import SwiftUI

extension AUI {

    public struct ContextItemListSubItemCell<Labels: View, Accessory: View>: View {
        @Environment(\.dynamicTypeSize) private var dynamicTypeSize
        @ScaledMetric private var uiScale: CGFloat = 1

        private let labels: Labels
        private let accessory: Accessory
        private let action: () -> Void

        public init(
            @ViewBuilder labels: () -> Labels,
            @ViewBuilder accessory: () -> Accessory = { SwiftUI.EmptyView() },
            action: @escaping () -> Void
        ) {
            self.labels = labels()
            self.accessory = accessory()
            self.action = action
        }

        public var body: some View {
            VStack(spacing: 0) {
                AUI.Divider()
                    .padding(.leading, leadingPadding)
                    .paddingStyle(.trailing, .standard)

                Button {
                    action()
                } label: {
                    HStack(alignment: .center, spacing: 0) {
                        VStack(alignment: .leading, spacing: 2) {
                            labels
                        }
                        .frame(maxWidth: .infinity, alignment: .leading)

                        accessory
                            .paddingStyle(.leading, .cellAccessoryPadding)
                    }
                    .paddingStyle(.top, .cellTop)
                    .paddingStyle(.bottom, .cellBottom)
                    .padding(.leading, leadingPadding)
                    .paddingStyle(.trailing, .standard)
                    .contentShape(Rectangle())
                }
                .background(.backgroundLightest)
                .buttonStyle(.tintedContextButton)
            }
        }

        private var leadingPadding: CGFloat {
            // cellIconLeading + scaledIconSize + cellIconText
            22 + uiScale.iconScale * 24 + 18
        }
    }
}

// MARK: - Preview

#if DEBUG

#Preview {
    PreviewContainer {
        let date = Date.now.dateTimeString

        AUI.ContextItemListCell(
            icon: { Image.assignmentLine.scaledIcon() },
            labels: {
                Text("Assignment 1").textStyle(.cellLabel)
                Text(date).textStyle(.cellLabelSubtitle)
            },
            isLastItem: nil,
            action: {}
        )

        AUI.ContextItemListSubItemCell(
            labels: {
                Text("Sub Item 1").textStyle(.cellLabel)
                Text(date).textStyle(.cellLabelSubtitle)
            },
            action: {}
        )

        AUI.ContextItemListSubItemCell(
            labels: {
                Text("Sub Item 2").textStyle(.cellLabel)
                Text(verbatim: .loremIpsumMedium).textStyle(.cellLabelSubtitle)
            },
            action: {}
        )

        AUI.ContextItemListSubItemCell(
            labels: {
                Text("Sub Item 3").textStyle(.cellLabel)
                Text(verbatim: .loremIpsumMedium).textStyle(.cellLabelSubtitle)
            },
            accessory: { AUI.DisclosureIndicator() },
            action: {}
        )

        AUI.Divider()

        AUI.ContextItemListCell(
            icon: { Image.assignmentLine.scaledIcon() },
            labels: {
                Text("Assignment 2").textStyle(.cellLabel)
                Text(date).textStyle(.cellLabelSubtitle)
            },
            isLastItem: true,
            action: {}
        )
    }
    .tint(.course10)
}

#endif
