...
#Preview(traits: .sizeThatFitsLayout) {
    ...
    func factory(title: String) -> some View {
        ...
        VStack(spacing: 20) {
            ...
            AUI.Toggle(isOn: $isOn1) { label }
            AUI.Toggle(isOn: $isOn2) { label }
            AUI.Divider()
            Text(verbatim: "Disabled")
            AUI.Toggle(isOn: .constant(true)) { label }.disabled(true)
            AUI.Toggle(isOn: .constant(false)) { label }.disabled(true)
            AUI.Divider()
            Text(verbatim: "Long Text")
            AUI.Toggle(isOn: .constant(true)) {
                Text(AUI.PreviewData.loremIpsumMedium)
            }.disabled(true)
        }
        .padding()
    }
