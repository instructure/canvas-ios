...
public enum ScreenState: Equatable, Hashable {
    case loading
    case error
    case empty
 ...
 */
public struct BaseScreenConfig {
...
public struct BaseScreen<Content>: View where Content: View {
...
#Preview("Loading") {
    BaseScreen(state: .loading) { _ in
        Text(verbatim: "Data")
    }
}

#Preview("Error") {
    BaseScreen(state: .error) { _ in
        Text(verbatim: "Data")
    }
}

#Preview("Empty") {
    BaseScreen(state: .empty) { _ in
        Text(verbatim: "Data")
    }
}
...
#Preview("Data With Loading Overlay") {
    BaseScreen(state: .data(loadingOverlay: true)) { _ in
        VStack(alignment: .leading) {
            ForEach(0..<100) { index in
                HStack {
                    Text(verbatim: "Data \(index)")
...
#Preview("Horizontal Scrollable Data") {
    BaseScreen(state: .data, config: .init(scrollAxes: .horizontal)) { _ in
        HStack {
...
#Preview("Vertical Small Data Not Refreshable") {
    BaseScreen(state: .data, config: .init(refreshable: false)) { _ in
        VStack(alignment: .leading) {
...
#Preview("Geometry Reader") {
    BaseScreen(state: .data) { geometry in
        VStack(spacing: 0) {
...
private final class ChangingViewModel: ObservableObject {
    ...
    private lazy var timer: Timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
        let allStates: [ScreenState] = [.loading, .error, .empty, .data, .data(loadingOverlay: true)]
