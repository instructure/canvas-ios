...
public class MockHTTPResponse: Codable {
    ...
    convenience init?<D: Codable>(value: D) {
        ...
        self.init(data: data,
                  http: HTTPURLResponse(url: .stub, statusCode: 200, httpVersion: nil, headerFields: nil))
    }
    ...
    public static var noContent: MockHTTPResponse {
        MockHTTPResponse(http: HTTPURLResponse(url: .stub, statusCode: 204, httpVersion: nil, headerFields: [:]))
    }
}
