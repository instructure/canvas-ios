...
public class K5ScheduleWeekViewModel: ObservableObject {
    ...
    private var courseInfoByCourseIDs: [String: (color: Color, image: URL?, isHomeroom: Bool, shouldHideScores: Bool)] = [:]
    ...
    private func setupCourseColors(_ courses: [Course]) {
        ...
        let courseInfoByCourseIDs = coursesByIDs.mapValues { (Color($0[0].color), $0[0].imageDownloadURL, isHomeroom: $0[0].isHomeroomCourse, shouldHideScores: $0[0].hideQuantitativeData) }
    ...
    private func makeMissingItems() -> [K5ScheduleEntryViewModel] {
        return missingSubmissions.map { assignment in
            let shouldHideScores = courseInfoByCourseIDs[assignment.course_id.rawValue]?.shouldHideScores == true
            ...
            return K5ScheduleEntryViewModel(leading: .warning,
                                            ...
                                            labels: [],
                                            score: shouldHideScores ? nil : score,
                                            dueText: dueText,
        ...
        for (subject, plannables) in plannablesBySubjects {
            let entries: [K5ScheduleEntryViewModel] = plannables.map { plannable in
                ...
                return K5ScheduleEntryViewModel(leading: .checkbox(isChecked: isCompleted),
                                                ...
                                                score: subject.shouldHideScores ? nil : plannable.k5SchedulePoints,
