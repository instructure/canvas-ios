...
public enum ScreenState: String, Equatable, Identifiable, CaseIterable {
    case loading
    case error
    case empty
    case data
 ...
 */
public struct BaseScreenConfig: Equatable {
...
class ChangingViewModel: ObservableObject {
    ...
    private lazy var timer: Timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
        self?.state = ScreenState.allCases.randomElement()!
    }
