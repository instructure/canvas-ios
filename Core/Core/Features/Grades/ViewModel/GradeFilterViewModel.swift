...
public final class GradeFilterViewModel: ObservableObject {
    ...
    private func mapGradingPeriod() {
        ...
        gradingPeriodOptions = .init(
            all: [GradingPeriod.Snapshot.optionItemAll] + gradingPeriods.map { $0.optionItem },
            initial: initialGradingPeriod?.optionItem ?? GradingPeriod.Snapshot.optionItemAll
        )
    }
...
private extension GradingPeriod.Snapshot {
