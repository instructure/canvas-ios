...
final class GradeListInteractorPreview: GradeListInteractor {
    ...
    func loadBaseData(ignoreCache: Bool) -> GradeListGradingPeriodData {
        GradeListGradingPeriodData(
            course: .init(model: Course.save(.make(), in: context)),
    ...
    func getGrades(
        ...
        GradeListData(
            ...
            currentGradingPeriod: .init(model: .save(.make(), courseID: "courseID", in: context)),
            totalGradeText: "80%"
        )
    }
