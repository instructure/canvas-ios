...
struct AssigmentAssigneeList: View {
    ...
    init(courseID: String, groupCategoryID: String?, selection: Binding<[Assignee]>, env: AppEnvironment) {
        ...
        GeometryReader { geometry in VStack(spacing: 0) {
            ...
            ScrollView {
                if isEmpty {
                    ...
                    ConditionallyLazyVStack(alignment: .leading, spacing: 0, pinnedViews: .sectionHeaders) { list }
                }
            }
                .background((isEmpty ? Color.backgroundLightest : Color.backgroundGrouped).edgesIgnoringSafeArea(.all))
        } }
        .navigationTitle(String(localized: "Add Assignee", bundle: .core), style: .modal)
    }
