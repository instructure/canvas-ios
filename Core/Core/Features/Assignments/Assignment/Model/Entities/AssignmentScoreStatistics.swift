...
public final class AssignmentScoreStatistics: NSManagedObject {
    ...
    public func update(fromApiModel item: APIAssignment.APIScoreStatistics, in client: NSManagedObjectContext) {
        mean = item.mean
        min = item.min
        max = item.max
    }
