    ...
    private func load(attachment: NSItemProvider, callback: @escaping (Result<URL, Error>) -> Void) {
        ...
        attachment.loadItem(forTypeIdentifier: uti.rawValue, options: nil) { data, error in
            guard let coding = data, error == nil else {
                RemoteLogger.shared.logError(name: "Failed to get encoded attachment data", reason: error?.localizedDescription)
            ...
            do {
                ...
                RemoteLogger.shared.logError(name: "Failed to copy shared file to container folder", reason: error.localizedDescription)
                callback(.failure(error))
            }
        }
    }
