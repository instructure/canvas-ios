...
public struct SwitchExperienceUseCase: APIUseCase {
    ...
    public let request: PostSwitchExperienceRequest

    public init(experience: String) {
        self.request = PostSwitchExperienceRequest(experience: experience)
        self.cacheKey = nil
    }

    public func write(
        response: PostSwitchExperienceRequest.Body?,
        urlResponse _: URLResponse?,
        to client: NSManagedObjectContext
    ) {
        guard let response else { return }
        CDExperienceSummary.update(experience: response.experience, in: client)
>>>>>>>> master:Core/Core/Features/Experience/Data/SwitchExperienceUseCase.swift
    }
