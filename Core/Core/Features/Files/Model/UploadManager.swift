...
open class UploadManager: NSObject, URLSessionDelegate, URLSessionTaskDelegate, URLSessionDataDelegate {
    ...
    private func submit(file: File, courseID: String, assignmentID: String, comment: String?, attempt: Int?) {
        ...
        announceFileUploadStatus(files: files) { [weak self] in
            ...
            let objectID = file.objectID
            process.performExpiringActivity(reason: "submit assignment") { [weak self] expired in
                guard let self else { return }
                ...
                self.submissionState.validate(for: requestable)
                            ...
                            self.context.performAndWait {
                                ...
                                guard let submission = response, error == nil else {
                                    Analytics.shared.logSubmission(
                                        ...
                                        additionalParams: self.submissionState.params()
                                    ...
                                    self.complete(file: file, error: error)
                                    self.submissionState.reportFailure()
                                    return
                                }
