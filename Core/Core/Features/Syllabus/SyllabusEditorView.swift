...
public struct SyllabusEditorView: View {
    ...
    public var body: some View {
        form
            ...
            .navigationTitle(String(localized: "Edit Syllabus", bundle: .core), style: .modal)
    ...
    func load() {
        ...
        useCase.modified(for: env).fetch(environment: env) { _, _, error in performUIUpdate {
    ...
    func save() {
        ...
        isSaving = true
        UpdateCourse(courseID: courseID, syllabusBody: html, syllabusSummary: showSummary)
            .modified(for: env)
            .fetch(environment: env) { result, _, error in
                performUIUpdate {
                    self.error = error
                    self.isSaving = false
                    if result != nil {
