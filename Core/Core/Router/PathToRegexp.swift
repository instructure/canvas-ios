...
func parse (_ str: String) -> [Any] {
    ...
    if let parserExpression = try? NSRegularExpression(pattern: parserPattern, options: .caseInsensitive) {
        let matches = parserExpression.matches(in: str, range: NSMakeRange(0, str.count))
...
func escapeGroup(_ group: String) -> String {
    ...
    guard let match = replacementRegExp.firstMatch(in: group, range: NSMakeRange(0, group.count)) else { return group }
...
func escapeString(_ str: String) -> String {
    guard let replacementRegExp = try? NSRegularExpression(pattern: "([\\.\\+\\*\\?=\\^!:\\$\\{\\}\\(\\)\\[]|\\/\\])", options: .caseInsensitive), let match = replacementRegExp.firstMatch(in: str, range: NSMakeRange(0, str.count)) else { return str }
    let range = match.range(at: 0)
...
public func extractParamsFromPath(_ path: String, match:NSTextCheckingResult, routePath:String) -> [String: String] {
    ...
    let routeMatches = parserExpression.matches(in: routePath, range: NSMakeRange(0, routePath.count))
    
    var params: [String: String] = [:]
    for (index, routeMatch) in routeMatches.enumerated() {
