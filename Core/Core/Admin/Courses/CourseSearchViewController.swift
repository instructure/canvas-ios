...
@available(iOS 13.0, *)
public class CourseSearchViewController: UIViewController, UISearchBarDelegate, UITableViewDelegate {
    enum Section {
        case courses
    }
    ...
    func updateResults() {
        ...
        switch SearchBy(rawValue: searchBySegmentedControl.selectedSegmentIndex) {
        case .courses, nil:
            searchBy = .course
        case .teachers:
            searchBy = .teacher
    ...
    @objc func searchByDidChange(_ control: UISegmentedControl) {
        let searchBy = SearchBy(rawValue: control.selectedSegmentIndex)
        switch searchBy {
        case .courses, nil:
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if let nextPage = nextPage, scrollView.isBottomReached() {
            self.nextPage = nil
