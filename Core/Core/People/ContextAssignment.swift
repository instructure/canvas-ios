...
public class ContextAssignment: NSManagedObject {
     ...
     */
    public var submission: ContextSubmission? {
        get {
        ...
        set {
            if let v = newValue {
                self.submissions = Set<ContextSubmission>( [v] )
            } else {
     ...
     */
    @NSManaged public var submissions: Set<ContextSubmission>?
...
extension ContextAssignment {
    func update(fromApiModel item: APIAssignment, in client: NSManagedObjectContext, updateSubmission: Bool, updateScoreStatistics: Bool) {
        ...
        if updateSubmission {
            if let values = item.submission?.values, values.count > 0 {
                self.submissions = Set<ContextSubmission>()
                let newSubs = values.map { ContextSubmission.save($0, in: client) }
                self.submissions = Set<ContextSubmission>(newSubs)
    ...
    public func requiresLTILaunch(toViewSubmission submission: ContextSubmission) -> Bool {
