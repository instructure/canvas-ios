...
class PathToRegexp_Tests: XCTestCase {
    ...
    func testCanMatchRoutes() {
        ...
        let numMatches = regexp.numberOfMatches(in: path, range: NSMakeRange(0, path.count))
    ...
    func testCanExtractParams() {
        ...
        guard let regexp = pathToRegexp(route), let match = regexp.firstMatch(in: path, range: NSMakeRange(0, path.count)) else { return XCTFail() }
