...
class UIColorTransformerTests: XCTestCase {

    func testTransformerRegistration() {
        // MARK: - GIVEN
        ValueTransformer.setValueTransformer(nil, forName: UIColorTransformer.name)
        for transformerName in ValueTransformer.valueTransformerNames() {
            XCTAssertFalse(ValueTransformer(forName: transformerName) is UIColorTransformer)
        }
        ...
        let isTransformerRegistered = ValueTransformer.valueTransformerNames().contains { name in
            ValueTransformer(forName: name) is UIColorTransformer
        }
