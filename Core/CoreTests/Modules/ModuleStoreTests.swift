...
class ModuleStoreTests: CoreTestCase {
    ...
    func testRefreshAPI() {
        ...
        api.mock(GetModulesRequest(courseID: "1", include: []), value: [.make()])
    ...
    func testRefreshAPIPaginated() {
        ...
        let request = GetModulesRequest(courseID: "1", include: [])
        api.mock(request, value: [.make(id: "1", position: 1, items: nil)], response: response)
        let task = api.mock(request.getNext(from: response)!, value: [.make(id: "2", position: 2, items: nil)])
        task.paused = true
        let store = ModuleStore(courseID: "1")
        store.delegate = self
        store.refresh(force: true)
    ...
    func testRefreshWhenItemsAreNilPaginated() {
        ...
        let response = HTTPURLResponse(next: link)
        let request = GetModuleItemsRequest(courseID: "1", moduleID: "1", include: [.content_details, .mastery_paths])
    ...
    func testForceRefreshDeletesCache() {
        ...
        let expectation = XCTestExpectation(description: "on change")
        onChange = expectation.fulfill
        api.mock(GetModulesRequest(courseID: "1", include: []), value: [.make(id: "2")])
        let store = ModuleStore(courseID: "1")
        store.delegate = self
        store.refresh(force: true)
        wait(for: [expectation], timeout: 1)
    ...
    func testRefreshRenewsTTL() {
        api.mock(GetModulesRequest(courseID: "1", include: []), value: [.make(items: nil)])
        let expectation = XCTestExpectation(description: "on change")
        expectation.expectedFulfillmentCount = 3
        onChange = expectation.fulfill
        let store = ModuleStore(courseID: "1")
