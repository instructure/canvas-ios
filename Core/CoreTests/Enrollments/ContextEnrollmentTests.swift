...
class ContextEnrollmentTests: CoreTestCase {
    func testEnrollmentStateInitRawValue() {
        let enrollment = ContextEnrollment.make(from: .make(enrollment_state: .invited))
    ...
    func testUpdateFromCourseAPI() {
        ...
        let model: ContextEnrollment = databaseClient.insert()
    ...
    func testUpdateFromEnrollmentsAPI() {
        ...
        let enrollment: ContextEnrollment = databaseClient.insert()
    ...
    func testIsStudentTeacherTA() {
        for type in [ "student", "StudentEnrollment", "StudentView" ] {
            let enrollment = ContextEnrollment.make(from: .make(type: type))
    ...
    func testFormattedRole() {
        XCTAssertEqual(ContextEnrollment.make(from: .make(role: "StudentEnrollment")).formattedRole, "Student")
        XCTAssertEqual(ContextEnrollment.make(from: .make(role: "TeacherEnrollment")).formattedRole, "Teacher")
        XCTAssertEqual(ContextEnrollment.make(from: .make(role: "TaEnrollment")).formattedRole, "TA")
        XCTAssertEqual(ContextEnrollment.make(from: .make(role: "ObserverEnrollment")).formattedRole, "Observer")
        XCTAssertEqual(ContextEnrollment.make(from: .make(role: "DesignerEnrollment")).formattedRole, "Designer")
        XCTAssertEqual(ContextEnrollment.make(from: .make(role: "Custom Role")).formattedRole, "Custom Role")
        let enrollment = databaseClient.insert() as ContextEnrollment
        enrollment.role = nil
    ...
    func testObservedUser() {
        ...
        let enrollment = ContextEnrollment.make(from: .make(observed_user: observedUser))
