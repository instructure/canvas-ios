...
class APIErrorTests: XCTestCase {
    ...
    func testFrom() {
        ...
        XCTAssertEqual(from(response: HTTPURLResponse(url: .stub, statusCode: 200, httpVersion: nil, headerFields: nil)), "There was an unexpected error. Please try again.")
        XCTAssertEqual(from(response: HTTPURLResponse(url: .stub, statusCode: 400, httpVersion: nil, headerFields: nil)), "There was an unexpected error. Please try again.")
    }
    ...
    func testUnauthorizedAPIError() {
        ...
        let response = HTTPURLResponse(url: .stub, statusCode: 401, httpVersion: nil, headerFields: nil)
    ...
    func testForbidden() {
        let response = HTTPURLResponse(url: .stub, statusCode: 403, httpVersion: nil, headerFields: nil)
        let error = APIError.from(data: nil, response: response, error: NSError.instructureError("default"))
    ...
    func testNotFound() {
        let response = HTTPURLResponse(url: .stub, statusCode: 404, httpVersion: nil, headerFields: nil)
        let error = APIError.from(data: nil, response: response, error: NSError.instructureError("default"))
    ...
    func testUnexpected() {
        let response = HTTPURLResponse(url: .stub, statusCode: 500, httpVersion: nil, headerFields: nil)
        let error = APIError.from(data: nil, response: response, error: NSError.instructureError("default"))
