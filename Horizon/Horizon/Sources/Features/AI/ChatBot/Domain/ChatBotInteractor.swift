...
enum AIContext {
    case chat(prompt: String = "", history: [ChatMessage] = []) // the user is chatting with the bot
    case chipFile(chipOption: ChipOption, file: File, history: [ChatMessage] = []) // the user has selected a chip while viewing a file
    case chipPage(chipOption: ChipOption, title: String, body: String, history: [ChatMessage] = []) // the user has selected a chip while viewing a page
    case file(prompt: String, file: File, history: [ChatMessage] = []) // the user is being shown a document (pdf, docx, etc)
    case page(prompt: String, title: String, body: String, history: [ChatMessage] = []) // the user is reading a page and types in a prompt

    var promptContextString: String {
        switch self {
        case .page(_, let title, let body, _),
             .chipPage(_, let title, let body, _):
            return "Here's the content they're referencing. The title of the document is \(title) and the body is \(body)"
        default:
             return ""
        }
    }
...
class ChatBotInteractorLive: ChatBotInteractor {
    ...
    private func classifierPrompt(prompt: String, context: AIContext, history: [ChatMessage]) -> AnyPublisher<ChatBotResponse, Error> {
        ...
        return basicChat(prompt: classifierPrompt, history: history)
            .flatMap(handleClassifierPromptResponse)
            .eraseToAnyPublisher()
    }

    private func handleClassifierPromptResponse(_ response: ChatBotResponse) -> AnyPublisher<ChatBotResponse, Error> {
    ...
    private func handleContextUpdate(userShortName: String, context: AIContext) -> AnyPublisher<ChatBotResponse, Error> {
        ...
        case .chat(let prompt, let history),
            ...
            return classifierPrompt(prompt: prompt, context: context, history: history)
    ...
    private func advancedChat() -> AnyPublisher<ChatBotResponse, Error> {
        JWTTokenRequest(.pine).api(from: canvasApi)
            .flatMap { pineApi in
