...
final class GetLearnCoursesInteractorMock: GetLearnCoursesInteractor {
    func getFirstCourse(ignoreCache: Bool) -> AnyPublisher<Horizon.LearnCourse?, any Error> {
        Just(LearnCourse(id: "ID", name: "Canvas Preview", enrollmentId: "enrollmentId"))
            .setFailureType(to: Error.self)
            .eraseToAnyPublisher()
    }
    func getCourses(ignoreCache: Bool) -> AnyPublisher<[Horizon.LearnCourse], Never> {
        Just(
            [
