...
public class AssignmentsHelper: BaseHelper {
    ...
    public static func createAssignment(
        ...
        published: Bool = true,
        submissionTypes: [SubmissionType] = [.online_text_entry]) -> DSAssignment {
        let assignmentBody = CreateDSAssignmentRequest.RequestDSAssignment(
            name: name, description: description + name, published: published, submission_types: submissionTypes)
