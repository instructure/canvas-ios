...
extension UILabel {
    func resizeHeightToFit(heightConstraint: NSLayoutConstraint) {
        ...
        let rect = text!.boundingRect(with: CGSize(width: frame.size.width, height: CGFloat.greatestFiniteMagnitude), options: .usesLineFragmentOrigin, attributes: attributes, context: nil)
...
enum RubricCellViewModel: TableViewCellViewModel {
    ...
    func cellForTableView(_ tableView: UITableView, indexPath: IndexPath) -> UITableViewCell {
        ...
        case .Title(let points, let pointsPossible):
            let cell = tableView.dequeueReusableCell(withIdentifier: "rubricPointsPossibleCell") as! PointsPossibleCell

            cell.pointsLabel.text = points == true ? "\(points) / \(pointsPossible)" : "\(pointsPossible)"
            
            cell.pointsTitleLabel.text = NSLocalizedString("Points Possible", tableName: "Localizable", bundle: Bundle(identifier: "com.instructure.AssignmentKit")!, value: "", comment: "Rubric Points Possible Title")
            
            return cell
        case .Criterion(let criterionDescription, let longDescription):
            guard let cell = tableView.dequeueReusableCell(withIdentifier: "RubricCriterionCell") else { fatalError("expected RubricCriterionCell") }
            
            cell.textLabel?.text = criterionDescription.uppercased()
            cell.detailTextLabel?.text = longDescription
            return cell
        case .CriterionRating( _, let comments, let points, let description, let selected):
            let cell = tableView.dequeueReusableCell(withIdentifier: "rubricCriterionRow") as! RubricCriterionRatingCell
            
            cell.lineViewHeight.constant = 1 / UIScreen.main.scale
            ...
            cell.commentsLabel.isHidden = comments.isEmpty
            ...
            if (selected) {
                ...
                cell.pointsLabel.layer.backgroundColor = UIColor.white.cgColor
    ...
    static func rubricDetails(baseURL: URL, rubric: Rubric) -> [RubricCellViewModel] {
        ...
        for criterion in rubric.rubricCriterions.sorted(by: {$0.position.intValue < $1.position.intValue}) {
            ...
            let criterionAssessment: RubricAssessment? = getAssessment(criterion: criterion, assessments: assessments)
            let assessmentRating: RubricCriterionRating? = getRating(criterion: criterion, assessment: criterionAssessment)
            
            //Turn each rating in the Criterion into cell view models
            cells += criterion.ratings.sorted(by: {$0.points.doubleValue > $1.points.doubleValue}).map({ rating -> RubricCellViewModel in
                ...
                if let criterionAssessment = criterionAssessment {
                    if let assessmentRating = assessmentRating {
                        ...
                        customCellViewModel = self.customCellViewModel(assessment: criterionAssessment)
                    }
                }
    ...
    static func customCellViewModel(assessment: RubricAssessment) -> RubricCellViewModel? {
        ...
        if (hasCustomGrade || hasComment) {
            let comment = NSLocalizedString("Comment", tableName: "Localizable", bundle: Bundle(identifier: "com.instructure.AssignmentKit")!, value: "", comment: "Title for a rating that hasn't been graded but has a comment")
            let noComment = NSLocalizedString("No Comment", tableName: "Localizable", bundle: Bundle(identifier: "com.instructure.AssignmentKit")!, value: "", comment: "Title for a rating that has custom graded but has no comment")
