...
struct Request<T>: CustomStringConvertible {
    ...
    init(auth: Session, method: TooLegit.Method, path: String, parameters: [String: Any]? = nil, parameterEncoding: ParameterEncoding? = nil, parseResponse: @escaping (Any?)->ResultType) {
        self.auth = auth
        self.method = method
    ...
    init(auth: Session, method: TooLegit.Method, url: URL, parameters: [String: Any]? = nil, parameterEncoding: ParameterEncoding? = nil, parseResponse: @escaping (Any?)->ResultType) {
        self.auth = auth
        self.method = method
        self.url = url
        self.parameters = parameters
...
*/
func makeRequest<T>(_ request: Request<T>, completed: @escaping (Result<Page<T>, NSError>)->()) -> URLSessionDataTask? {
    ...
    if let urlRequest = encodingResult.value {
        let dataTask = request.auth.URLSession.dataTask(with: urlRequest as URLRequest) { data, response, error in
            if let data = data, let httpURLResponse = response as? HTTPURLResponse {
                ...
                DispatchQueue.main.async {
                    let fallbackError = NSError(domain: "com.instructure", code: 0, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("There was no data!", tableName: "Localizable", bundle: Bundle(identifier: "com.instructure.QuizKit")!, value: "", comment: "")])
                    let error = (error as? NSError) ?? fallbackError
                    completed(Result<Page<T>, NSError>(error: error))
                }
            }
        }
        dataTask.resume()
...
private func infoForJSON(_ json: Any?) -> [String: Any] {
    ...
    var info: [String: Any] = [:]
    let localizedDescription = NSLocalizedString("There was a problem with the Canvas request.", tableName: "Localizable", bundle: Bundle(identifier: "com.instructure.QuizKit")!, value: "", comment: "generic error message")
