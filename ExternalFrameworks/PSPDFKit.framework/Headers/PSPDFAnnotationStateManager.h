...
/// Used to customize the image quality options we offer for adding image annotations.
...
typedef NS_ENUM(NSUInteger, PSPDFAnnotationStateManagerStylusMode) {
    /// Use stylus if the global stylus manager is using Apple Pencil.
    PSPDFAnnotationStateManagerStylusModeFromStylusManager,
    /// Annotate with a single direct (finger) touch. Scroll with two fingers.
    PSPDFAnnotationStateManagerStylusModeDirect,
    /// Annotate only with Apple Pencil. Tap and scroll with fingers as normal.
...
- (BOOL)annotationStateManager:(PSPDFAnnotationStateManager *)manager shouldChangeState:(nullable PSPDFAnnotationString)currentState to:(nullable PSPDFAnnotationString)proposedState variant:(nullable PSPDFAnnotationString)currentVariant to:(nullable PSPDFAnnotationString)proposedVariant;
...
- (void)annotationStateManager:(PSPDFAnnotationStateManager *)manager didChangeState:(nullable PSPDFAnnotationString)oldState to:(nullable PSPDFAnnotationString)newState variant:(nullable PSPDFAnnotationString)oldVariant to:(nullable PSPDFAnnotationString)newVariant;
...
@property (nonatomic, copy, nullable) PSPDFAnnotationString variant;

/// Sets the annotation type and variant at the same time.
/// @see state, variant
- (void)setState:(nullable PSPDFAnnotationString)state variant:(nullable PSPDFAnnotationString)variant;
...
- (void)toggleState:(nullable PSPDFAnnotationString)state variant:(nullable PSPDFAnnotationString)variant;

/// String identifier used as the persistence key for the current combination of state and variant.
@property (nonatomic, copy, readonly) PSPDFAnnotationString stateVariantIdentifier;
...
/**
 Whether compatible annotations types may be created only with touches of type `UITouchTypeStylus`. If set to
 ...
 The default is `fromStylusManager`, which effectivly uses one of the other two modes depending on if the global
 `PSPDFStylusManager` is using the Apple Pencil.
 */
...
- (BOOL)isHighlightAnnotationState:(nullable PSPDFAnnotationString)state;
