...
/// @note Will always be called on the main thread.
- (void)documentSharingViewController:(PSPDFDocumentSharingViewController *)shareController didFinishWithSelectedOptions:(PSPDFDocumentSharingOptions)selectedSharingOption files:(NSArray<PSPDFFile *> *)files annotationSummary:(nullable NSAttributedString *)annotationSummary error:(nullable NSError *)error;
...
/// @note If you implement this method but return nil the default text will be used instead. Return an empty string if you want to display no text. Will always be called on the main thread.
...
///       shared files are compatible with the passed in files. Will always be called from the main thread.
///
