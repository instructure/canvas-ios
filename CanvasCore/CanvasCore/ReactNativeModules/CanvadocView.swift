...
public class CanvadocView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        openInButton.setTitle(NSLocalizedString("Open in…", bundle: .canvas, comment: ""), for: .normal)
        ...
        task = sessionAPI?.makeRequest(request) { [weak self] _, response, error in
            ...
            if let error = error {
                return DispatchQueue.main.async {
                    ...
                    let dismiss = NSLocalizedString("Dismiss", bundle: .canvas, comment: "")
                    alert.addAction(UIAlertAction(title: dismiss, style: .default, handler: nil))
                    AppEnvironment.shared.topViewController?.present(alert, animated: true, completion: nil)
                }
            }
