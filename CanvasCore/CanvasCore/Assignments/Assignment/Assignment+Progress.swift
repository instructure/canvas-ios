...
extension Assignment {
    ...
    static func invalidateCaches(_ progress: Progress, scope: RefreshScope, context: NSManagedObjectContext) {
        ...
        case (.contributed, .discussion):
            if let assignment: Assignment = (((try? context.findOne(withValue: progress.itemID, forKey: "discussionTopicID") as? Assignment) as Assignment??)).flatMap({ $0 }) {
                invalidateDetail(progress.contextID, assignmentID: assignment.id)
            }
            
        case (.submitted, .quiz):
            if let assignment: Assignment = (((try? context.findOne(withValue: progress.itemID, forKey: "quizID") as? Assignment) as Assignment??)).flatMap({ $0 }) {
                invalidateDetail(progress.contextID, assignmentID: assignment.id)
            }
            
        case (.submitted, .externalTool):
            if let assignment: Assignment = (((try? context.findOne(withValue: progress.itemID, forKey: "url") as? Assignment) as Assignment??)).flatMap({$0}) {
                invalidateDetail(progress.contextID, assignmentID: assignment.id)
            }
