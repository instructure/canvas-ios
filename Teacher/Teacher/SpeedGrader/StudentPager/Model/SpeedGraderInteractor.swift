    ...
    func loadInitialData() {
        loadAssignment()
            ...
            .sink { [weak self] completion in
                ...
                let focusedSubmissionIndex = submissions.firstIndex { self.userID == SpeedGraderAllUsersUserID || $0.userID == self.userID }
                let data = SpeedGraderInteractorState.loaded(
                    assignment: assignment,
                    submissions: submissions,
                    focusedSubmissionIndex: focusedSubmissionIndex ?? 0
                )
