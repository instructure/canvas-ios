 ...
 * promises:
 *
 *   - When a FSA action is received with a payload that is a promise, the
 *     action is tagged with a `pending: true` field.
...
let promiseMiddleware: MiddlewareAPI = ({ dispatch }) => {
  return next => action => {
    if (isFSA(action) && isPromise(action.payload) && !action.error) {
      action.payload.then(
