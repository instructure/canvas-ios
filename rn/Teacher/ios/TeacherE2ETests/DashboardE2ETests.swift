...
class DashboardE2ETests: CoreUITestCase {
    func testDashboardE2E() {
        ...
        InboxHelper.Filter.byCourse.waitUntil(condition: .visible)
        DashboardHelper.TabBar.dashboardTab.hit()
        XCTAssertTrue(DashboardHelper.courseCard(courseId: "263").waitUntil(condition: .visible).isVisible)
        app.find(label: "Edit").hit()
        DashboardHelper.backButton.hit()
        XCTAssertTrue(DashboardHelper.courseCard(courseId: "263").waitUntil(condition: .visible).isVisible)
        XCTAssertTrue(DashboardHelper.courseCard(courseId: "5586").waitUntil(condition: .visible).isVisible)
        XCTAssertTrue(DashboardHelper.courseCard(courseId: "892").waitUntil(condition: .visible).isVisible)
        XCTAssertTrue(DashboardHelper.courseCard(courseId: "399").waitUntil(condition: .visible).isVisible)
    }
}
