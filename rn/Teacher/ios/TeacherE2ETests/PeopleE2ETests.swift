...
class PeopleE2ETests: CoreUITestCase {
    func testPeopleE2E() {
        ...
        XCTAssertTrue(app.find(labelContaining: "Assignments").waitUntil(condition: .visible).isVisible)
        ...
        XCTAssertEqual(PeopleHelper.ContextCard.submissionsMissingLabel.waitUntil(condition: .visible).label, "1 missing")
        PeopleHelper.backButton.hit()
        app.find(label: "Filter").hit()
        app.find(label: "Teachers").hit()
        app.swipeDown()
        XCTAssertFalse(app.find(label: "Student One").waitUntil(condition: .vanish).isVisible)
        XCTAssertTrue(app.find(label: "Teacher One").waitUntil(condition: .visible).isVisible)
    }
}
