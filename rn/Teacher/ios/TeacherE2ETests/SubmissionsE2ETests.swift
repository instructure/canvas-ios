...
class SubmissionsE2ETests: CoreUITestCase {
    func testSubmissionsE2E() {
        ...
        XCTAssertTrue(app.find(labelContaining: "Student One").waitUntil(condition: .visible).isVisible)

        app.find(labelContaining: "Filter").hit()
        app.find(labelContaining: "Needs Grading").hit()
        app.find(label: "Done").hit()
        XCTAssertTrue(app.find(labelContaining: "No Submissions").waitUntil(condition: .visible).isVisible)

        app.find(labelContaining: "Filter").hit()
        app.find(labelContaining: "Not Submitted").hit()
        app.find(label: "Done").hit()
        pullToRefresh()
        XCTAssertTrue(app.find(labelContaining: "Test Student").waitUntil(condition: .visible).isVisible)
        XCTAssertTrue(app.find(labelContaining: "Student Two").waitUntil(condition: .visible).isVisible)
        XCTAssertFalse(app.find(labelContaining: "Student One").waitUntil(condition: .vanish).isVisible)
        ...
        XCTAssertTrue(app.find(labelContaining: "This assignment is for testing module navigation").waitUntil(condition: .visible).isVisible)
