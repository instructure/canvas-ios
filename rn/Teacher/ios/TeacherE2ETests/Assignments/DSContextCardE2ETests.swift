...
class DSContextCardE2ETests: E2ETestCase {
    func testContextCardE2E() {
        ...
        XCTAssertTrue(PeopleHelper.ContextCard.userNameLabel.waitUntil(condition: .visible).hasLabel(label: student.name, strict: false))
        XCTAssertTrue(PeopleHelper.ContextCard.courseLabel.waitUntil(condition: .visible).hasLabel(label: course.name, strict: false))
        XCTAssertTrue(PeopleHelper.ContextCard.sectionLabel.waitUntil(condition: .visible).hasLabel(label: "Section: \(course.name)", strict: false))
        XCTAssertTrue(PeopleHelper.ContextCard.submissionsTotalLabel.waitUntil(condition: .visible).hasLabel(label: "3 submitted", strict: false))
        XCTAssertTrue(PeopleHelper.ContextCard.submissionsLateLabel.waitUntil(condition: .visible).hasLabel(label: "0 late", strict: false))
        XCTAssertTrue(PeopleHelper.ContextCard.submissionsMissingLabel.waitUntil(condition: .visible).hasLabel(label: "0 missing", strict: false))

        let submissionCell1 = PeopleHelper.ContextCard.submissionCell(assignment: assignment).waitUntil(condition: .visible)
        XCTAssertTrue(submissionCell1.hasLabel(label: "Submission \(assignment.name), Submitted, NEEDS GRADING", strict: false))

        let submissionCell2 = PeopleHelper.ContextCard.submissionCell(assignment: assignment2).waitUntil(condition: .visible)
        XCTAssertTrue(submissionCell2.hasLabel(label: "Submission \(assignment2.name), Submitted, grade 7 / 10", strict: false))

        let submissionCell3 = PeopleHelper.ContextCard.submissionCell(assignment: assignment3).waitUntil(condition: .visible)
        XCTAssertTrue(submissionCell3.hasLabel(label: "Submission \(assignment3.name), Submitted, grade 5 / 5", strict: false))
