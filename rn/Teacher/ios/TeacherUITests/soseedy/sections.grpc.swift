...
internal enum Soseedy_SeedySectionsServerError : Error {
  case endOfStream
}

/// To build a server, implement a class that conforms to this protocol.
internal protocol Soseedy_SeedySectionsProvider {
...
internal class Soseedy_SeedySectionsSession {
...
// CreateSection (Unary)
internal class Soseedy_SeedySectionsCreateSectionSession : Soseedy_SeedySectionsSession {
  private var provider : Soseedy_SeedySectionsProvider

  /// Create a session.
  fileprivate init(handler:gRPC.Handler, provider: Soseedy_SeedySectionsProvider) {
  ...
  fileprivate func run(queue:DispatchQueue) throws {
    try handler.receiveMessage(initialMetadata:initialMetadata) {(requestData) in
      if let requestData = requestData {
        let requestMessage = try Soseedy_CreateSectionRequest(serializedData:requestData)
        let replyMessage = try self.provider.createsection(request:requestMessage, session: self)
...
internal class Soseedy_SeedySectionsServer {
  ...
  internal init(address:String,
              provider:Soseedy_SeedySectionsProvider) {
    gRPC.initialize()
    ...
    server.run {(handler) in
      ...
      do {
        ...
        case "/soseedy.SeedySections/CreateSection":
          try Soseedy_SeedySectionsCreateSectionSession(handler:handler, provider:provider).run(queue:queue)
        default:
