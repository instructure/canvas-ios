...
folders = %w[CanvasCore Core Frameworks Canvas Parent Teacher Student TestsFoundation rn]
files = []
...
files.each do |file|
    ...
    # Assume new files were authored this year.
    # Git takes a long time to determine first commit, and often gets it wrong.
    authored_year = Time.new.year.to_s()
    # If the existing copyright date is older, use that.
    /\/\/.*copyright.*(\d{4})/i.match(text) { |m|
        if authored_year == "" || authored_year > m[1]
            authored_year = m[1]
        end
    }
