...
Generates two Swift files from the rebrandLight.json token tree:
  ...
  packages/InstUI/Sources/Semantic/Generated/InstUI.Semantic.Colors.swift
    — Nested Swift struct hierarchy extending InstUI.Semantic (compile-time type safety, no values)

  packages/InstUI/Sources/Semantic/Generated/InstUI.Semantic.Colors+Build.swift
    — Runtime build() function that reads JSON + resolves token references
*/
...
function generateStructFile(colorTree) {
  ...
  lines.push('')
  lines.push(genStructBlock('Colors', colorTree, 1))
  lines.push('}')
  lines.push('')
  return lines.join('\n')
}
