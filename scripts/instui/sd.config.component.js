...
//   • Typed accessor extensions (CGFloat.iui, Color.iui, etc.) — primary public API, listed first
...
function generateComponentFile({ typeName, tree, fileResource, rootKey }) {
  ...
  // Placed first as these are the primary public API for call sites.
  ...
  lines.push(`        let resolver = InstUI.Component.Resolver(theme: theme, leaves: leaves)`)
...
function generateThemeComponentsFile(components, outputDir) {
  ...
  lines.push('        let base = InstUI.Component.Resolver(')
  ...
  lines.push('            _ build: (InstUI.Component.Resolver) throws -> T')
  lines.push('        ) throws -> T {')
  ...
  lines.push('            fontWeights: theme.fontWeight,')
  lines.push('            fontFamilies: theme.fontFamily')
  lines.push('        )')
  lines.push('    }')
  lines.push('}')
  lines.push('')
